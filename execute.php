...
if($type == 'section') {
    echo install_section($sectionid, $cmid);
} else {
...
function install_section($target_sid, $template_sid) {
    ...
    // get target course from sectionid
//    $courseid = $DB->get_field('course_sections', 'course', array('id' => $target_sid));
    ...
    return 'Sections ID '.$template_sid.' will be installed after target section ID '.$target_sid.' here...';
}

//----------------------------------------------------------------------------------------------------------------------
function install_section_modules($targetsectionid, $sid) {
    global $CFG, $DB, $USER;
    ...
    // get target course from sectionid
    $courseid = $DB->get_field('course_sections', 'course', array('id' => $targetsectionid));
    $course = $DB->get_record('course', array('id' => $courseid));
    ...
    // Now restore the section after the given target
    $targetsection = $DB->get_record('course_sections', array('id' => $targetsectionid));
    $sectionnum = $targetsection->section;
    ...
    return 'Sections ID '.$sid.' will be installed after target section ID '.$targetsectionid.' here...';
}
